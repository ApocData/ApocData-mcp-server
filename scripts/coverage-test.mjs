...
/**
 * 覆盖测试：
 * 1. 截断：传 limit=超上限，验证响应头 X-Tdc-Truncated=true
 * 2. 限流：连续调 N 次，看 X-Tdc-RateLimit-Remaining 是否递减
...
// daily 上限 30，传 999 应触发截断
{
  ...
  const truncated = r.headers['x-tdc-truncated'];
  check(truncated ? 'PROD' : 'LAG', 'daily limit=999 触发 X-Tdc-Truncated 头',
...
// 连续打 5 次 quote，看 remaining 是否递减
{
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const rem = r.headers['x-tdc-ratelimit-remaining'];
    if (rem !== undefined) remainings.push(parseInt(rem, 10));
  }
  if (remainings.length === 0) {
    check('LAG', '限流头 X-Tdc-RateLimit-Remaining 存在', false, 'no header in any of 5 responses');
  } else {
